def some_new_function():
    print('some_new_function --masterVersion -newVersion')

def new_add_function():
    print('add')

def some_other_new_function():
    print('some_other_new_function, again -- hotfixVersion')

def first_commit_function():
    print('some function created during the first commit');

def second_commit_function():
    print('some function created during the second commit');

def second_branch():
    print('second branch')

def third_branch():
    print('third branch - change made in branch_one, modified by branch_two')